...
dataset = pd.read_csv('./wangdata/All_unbalance_15.csv')  #平衡数据集 7万条
...
labelset = pd.read_csv('./wangdata/Label_15_imbalanced.csv')      #不平衡数据集label
...
classifier.add(Dense(output_dim = 128, init = 'uniform', activation = 'relu', input_dim = 1480))
...
checkpointer = ModelCheckpoint(filepath="./MLPmodel/MLP_model_15APP_balanced_cpu.h5",
                               verbose=0,
                               save_best_only=True)
tensorboard = TensorBoard(log_dir='./MLPmodel/balanced',
                          histogram_freq=0,
                          write_graph=True,
                          write_images=True)
...
classifier.save('./MLPmodel/Smote-MLPmodel.h5')
